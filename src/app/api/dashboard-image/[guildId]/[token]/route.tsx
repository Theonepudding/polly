...
export async function GET(
  ...
  const hasIds  = url.searchParams.has('ids')
  const idParam = url.searchParams.get('ids') ?? ''
  const pollIds = idParam.split(',').map(s => s.trim()).filter(Boolean)

  // When `ids` param is present (even empty), trust it — empty means zero active polls.
  // Only fall back to getPolls() when the param is absent entirely (legacy/direct URL access).
